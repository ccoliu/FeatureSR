...
**Changelog (v2.5 → v2.6)**: Ran the isolation ablation flagged as open in v2.5's §6.2 — re-measured the ISIC 2018 Feature-SR+Ensemble TTA comparison at $\lambda_3=0$ (`reports/0913b_ISIC_TTA_Lambda3隔離驗證報告.md`) to test whether $\mathcal{L}_{\text{CR}}$ causes the regression. Result: the regression got *larger* and newly significant (-1.74%, vs. -0.85% non-significant at $\lambda_3=0.3$), refuting rather than confirming the hypothesis. §6.2 rewritten accordingly: the $\mathcal{L}_{\text{CR}}$ explanation is retracted, the true cause is left unidentified, and the practical recommendation (disable TTA for this configuration on ISIC 2018) is retained on firmer evidence (2 of 3 tested configurations now show a statistically real regression, not 1 of 2).

**Changelog (v2.6 → v2.7)**: Full consistency pass over the whole document after the v2.4→v2.6 revisions, fixing four issues found by re-reading it end to end. (1) §2 and §2.2 still presented FeatureRefiner as an active pipeline stage, contradicting §5/§6.3 — both now carry an explicit "disabled in the reported architecture" note, and §2.2 states what the stage reduces to at $L=0$ ($\text{LN}_{\text{post}}$ + $\ell_2$ normalization, 1,024 parameters). (2) §4.5's claim that $\mathcal{L}_{\text{CR}}$ acts as an incidental regularizer preventing ISIC's late-training collapse is **retracted**: the $\lambda_3=0$ run from the v2.6 isolation experiment shows the same stabilization with $\mathcal{L}_{\text{CR}}$ switched off (45.43% peak, 42.78% at epoch 100, no collapse), so the effect belongs to the Feature-SR branch as a whole, not to the cross-resolution objective — this is the second $\mathcal{L}_{\text{CR}}$-specific attribution in this document to fail direct testing, after §6.2's. (3) §4.7 described Feature-SR as giving back "roughly half" of the Ensemble-only gain on ISIC/ChestX; the actual figures are closer to two-thirds (+1.94%→+0.54%, +1.40%→+0.47%). (4) §6.1's "ChestX consistently shows the smallest gain" no longer holds for the combined configuration under the v2.5 numbers (CropDiseases' +0.08% is now smaller than ChestX's +0.10%); reworded to "among the weakest-responding domains" with the per-configuration ranking stated explicitly. No benchmark numbers changed in this version.
...
> **Architecture note (v2.5 onward)**: the **FeatureRefiner stage described in §2.2 is disabled in the reported architecture** (`sr_refiner_layers=0`) — an internal ablation found it contributes nothing measurable while costing 41.6% of FeatureSR's added parameters (§6.3). It is retained in this section because it is part of the implemented module and of every pre-v2.5 result, but every benchmark number in §4 comes from the configuration **without** it. With `sr_refiner_layers=0`, the stage between Upsampler and $\mathbf{F}_{\text{sr}}$ reduces to the final $\text{LN}_{\text{post}}$ + $\ell_2$ normalization in Eq. §2.2 (1,024 parameters), and the pipeline below should be read with box 2's self-attention and MLP lines omitted.
...
### 2.2 FeatureRefiner *(disabled in the reported architecture — see §6.3)*
As originally designed, to capture long-range contextual spatial dependencies among the $N=784$ upsampled tokens, the sequence passes through $L=2$ Pre-LN Transformer blocks with Multi-Head Self-Attention ($h=8$ heads):
...
In the reported architecture $L=0$, so $\mathbf{X}^{(L)} = \mathbf{F}_{\text{up}}$ (the Upsampler's output passes through untouched) and this equation reduces to $\text{LN}_{\text{post}}$ + $\ell_2$ normalization alone.
...
Across every experiment in this document, ChestX is consistently among the weakest-responding domains for either enhancement (Feature-SR: +0.27%, the smallest of the four domains at 5-shot; Prompt Ensemble: −0.09%, the only negative of the four; combined: +0.10%, second-smallest after CropDiseases' +0.08%). Cross-referencing the CC-CDFSL paper's [1] own Appendix Table 9 (comparison against SOTA CDFSL methods, 5-way 5-shot) shows this is not specific to FeatureSR or the prompt module:
...
### 6.2 ISIC 2018: TTA and Feature-SR Show a Real, Persistent Negative Interaction — the $\mathcal{L}_{\text{CR}}$ Hypothesis Is Refuted

*Updated in v2.6.* Three configurations of the same underlying comparison (Feature-SR + Ensemble checkpoint, no TTA vs. +TTA, ISIC 2018, 5-way 5-shot, 400 episodes) have now been measured:

| Configuration | No TTA | +TTA | $\Delta$ | CI Sum | Significant? |
|:---|:---:|:---:|:---:|:---:|:---:|
| `refiner=2`, $\lambda_3=0.3$ (originally reported) | 44.13 ± 0.67% | 42.75 ± 0.69% | -1.38% | 1.36% | ✅ yes |
| `refiner=0`, $\lambda_3=0.3$ (§6.3 architecture) | 45.48 ± 0.73% | 44.63 ± 0.62% | -0.85% | 1.35% | no |
| `refiner=0`, $\lambda_3=0$ (`reports/0913b_...md`) | 45.54 ± 0.64% | 43.80 ± 0.66% | **-1.74%** | 1.30% | **✅ yes — largest of the three** |

The original hypothesis (§6.2 in v2.5 and earlier) was that $\mathcal{L}_{\text{CR}}$'s cross-resolution consistency objective over-tightens the visual embedding around ISIC's training-time orientation statistics, making flipped views at inference destabilizing. This predicts the regression should shrink or disappear at $\lambda_3=0$. It does the opposite: removing $\mathcal{L}_{\text{CR}}$ entirely produced the *largest and most statistically clear* regression of the three configurations (-1.74%, exceeding its own 1.30% threshold). **The $\mathcal{L}_{\text{CR}}$ hypothesis is therefore refuted, not merely narrowed** — whatever causes TTA to hurt this specific configuration on ISIC 2018, it is not the Cross-Resolution Consistency Loss.

The true cause remains unidentified and is not pursued further in this document, since (a) this is a boundary/limitation statement, not a claim requiring root-cause attribution to be actionable, and (b) the practical decision (below) does not depend on knowing why. Plausible candidates outside the scope of this investigation: ISIC's dermoscopic lesion images may carry orientation-relevant diagnostic cues (contradicting the "flips are semantically safe" assumption made in earlier drafts of this section), or the interaction may involve the Ensemble-aware training objective rather than Feature-SR specifically (the Ensemble-only, non-Feature-SR configuration shows a much smaller, non-significant TTA effect on ISIC, -0.07%, per `reports/0901_TTA驗證報告.md` — the regression only appears once Feature-SR's FeatureUpsampler is also present, whatever its exact mechanism).

**Practical implication (unchanged in conclusion, now on firmer footing)**: `--use_tta` should remain disabled for ISIC 2018 in the Feature-SR + Ensemble configuration. Unlike the v2.5 text, this is no longer a "mild caution" — two of three tested configurations show a statistically real regression, and the one exception (`refiner=0`, $\lambda_3=0.3$) still has the same-sign point estimate. See `reports/0901_TTA驗證報告.md` for the original full verification and `reports/0913b_ISIC_TTA_Lambda3隔離驗證報告.md` for the $\mathcal{L}_{\text{CR}}$ isolation experiment.

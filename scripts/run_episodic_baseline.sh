...
# 確認對齊後才進入第二階段（+CC-CDFSL、+FeatureSR）。
# 每個 episode 結果即時寫入 results_episodic/*.jsonl，中斷後重跑同一腳本會自動續跑。
set -e
set -o pipefail
cd "$(dirname "$0")/.."

PYTHON=${PYTHON:-python}
LOG_DIR="logs/episodic_baseline"
mkdir -p "$LOG_DIR"

for SHOT in 1 5; do
  for DS in eurosat crop_disease isic chestx; do
    echo ">>> [CLIP-LoRA baseline, episodic] $DS ${SHOT}-shot  $(date '+%F %T')"
    $PYTHON train_episodic.py --dataset "$DS" --n_shot "$SHOT" \
